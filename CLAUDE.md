...
### App 截圖排版規範（`.shot-*`，2026-09-22 訂定）

放 App 截圖時**不要**沿用 `.work-img-grid`（那是給尺寸相近的作品圖用的）。App 截圖的形狀差異很大——
卡片特寫是很寬很扁的長條（例如 1076×146）、彈窗接近方形、整頁截圖是很長的直式（1440×3006）、
手機版更極端（390×2600）。全部丟進同一個兩欄格線會出現三個問題：長條圖被縮到字看不清楚、
長截圖把版面撐到爆、圖說長度不一讓兩欄高度參差。改用這套（CSS 在 `css/work-detail.css` 最下方）：

| 情況 | 用法 | 說明 |
|------|------|------|
| 原圖寬 ≥ 1000px | `.shot-list` > `figure.shot` | 一張獨佔一列滿版（內容欄 800px），縮下來仍看得清楚 |
| 換算後高度 > 620px | 框加 `.shot__frame--scroll` | 框內限高可捲動，不讓單張長圖洗版；圖說結尾註明「（框內可上下捲動看完整畫面）」 |
| 原圖寬 ≤ 600px | `.shot-pair` | 兩張並排，各約 390px，接近原尺寸最清楚 |
| 手機版截圖（390px 寬） | `.shot-phones` | 自動一排兩張，剛好 1:1 原尺寸 |
| 任何情況 | 都不要把圖放大超過 1.15 倍 | 小圖用 `figure` 的 inline `max-width` 限制寬度並置中，硬拉滿版會糊掉 |

圖說一律用 `<figcaption class="shot__cap">`（**不要用 `<p>`**，燈箱是靠 `figure > figcaption` 取圖說的），
格式統一成「**短標題**｜一到兩句說明」，視覺節奏才一致。`js/main.js` 的燈箱選擇器已涵蓋 `.shot__frame img`。
